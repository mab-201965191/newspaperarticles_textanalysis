---
title: "keyword_search_algorithm_text"
...
```{r}
keyword_corpus <- corpus(keyword, text_field = "text") 
...
write.csv(predictions, 'text_predictions_keywordsearch.csv', row.names=FALSE)
```
...
dfm_search_keywords <- dfm_search %>%
  dfm_trim(min_docfreq = 0.002, docfreq_type = "prop", verbose = TRUE)
# Keywords occuring in more than 35 documents
```
...
write.csv(agg_df, 'text_wordproportions_target_search.csv', row.names=FALSE)

write.csv(top_50_keywords_prop, "text_top50keywords_proportion.csv", row.names = FALSE)
```
...
write.csv(top_50_keywords_keyness, "text_top50keywords_keyness.csv", row.names = FALSE)
```
