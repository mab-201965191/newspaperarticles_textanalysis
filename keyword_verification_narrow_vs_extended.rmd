...
find_similarities <- function(list_of_dfs, search_type = "general", query_field = 'text', 
  ...
  saved$query <- query
  saved$df <- "general"
  saved$narrow_total <- 0
  saved$extended_total <- 0 
  saved$narrow_in_extended <- 0
  saved$extended_in_narrow <- 0
  saved$exact <- 0
  saved$narrow_notin_extended <- 0
  saved$extended_notin_narrow <- 0
  saved <- as.data.frame(saved)
  indx <- 1
...
```{r}
queries_two <- c("famine", "earthquake", "cyclone", "hurricane")
results_two <- find_word(list_dfs_gensearch, list_names_gensearch, query = queries_two)
results_two %>%
  group_by(query) %>%
  mutate(diff=count-lag(count,default=first(count))) %>%
  filter(diff < 0)
...
```{r}
# Look at difference in articles captures about Bangladesh and about 
